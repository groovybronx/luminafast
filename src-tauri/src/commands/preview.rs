...
pub async fn init_preview_service(_app_handle: &AppHandle) -> Result<(), String> {
    ...
    PREVIEW_SERVICE.set(service_arc)
...
pub async fn generate_preview(
    ...
    service.generate_preview(&path, preview_type, &source_hash)
        .await
...
pub async fn generate_batch_previews(
    ...
    service.generate_batch_previews(files_with_paths, preview_type)
        .await
...
pub async fn generate_preview_pyramid(
    ...
    for preview_type in [PreviewType::Thumbnail, PreviewType::Standard, PreviewType::OneToOne] {
        let result = service.generate_preview(&path, preview_type, &source_hash)
            .await
...
pub async fn cleanup_preview_cache(
    ...
    service.cleanup_cache(&cleanup_config)
        .await
...
pub async fn remove_preview(
    ...
    service.remove_preview(&source_hash, &preview_type)
        .await
...
pub async fn get_preview_path(
    ...
    if service.is_preview_cached(&source_hash, &preview_type).await {
        ...
        if let Some(path) = service.get_cached_preview_path(&source_hash, &preview_type).await {
...
pub async fn generate_previews_with_progress(
    ...
    for (index, preview_type) in preview_types.iter().enumerate() {
        ...
        let _ = app_handle.emit("preview_progress", serde_json::json!({
        ...
        let stats = service.generate_batch_previews(files_with_paths.clone(), *preview_type)
            .await
...
pub async fn benchmark_preview_generation(
    ...
    for i in 0..iterations {
        ...
        let result = service.generate_preview(&path, preview_type, test_hash)
            .await
    ...
    log::info!("Benchmark terminé: {} previews en {:?}", iterations, total_time);
