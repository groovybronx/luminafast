...
impl RawFormat {
    ...
    pub fn magic_bytes(&self) -> &'static [u8] {
        match self {
            // RAW formats (official signatures)
            RawFormat::CR3 => &[0x00, 0x00, 0x00, 0x18, 0x66, 0x74, 0x79, 0x70, 0x63, 0x72, 0x33, 0x20], // ftyp cr3 (ISO BMFF)
            RawFormat::CR2 => &[0x49, 0x49, 0x2A, 0x00], // TIFF LE
            RawFormat::NEF => &[0x49, 0x49, 0x2A, 0x00], // TIFF LE (Nikon)
            RawFormat::ARW => &[0x49, 0x49, 0x2A, 0x00], // TIFF LE (Sony)
            RawFormat::RAF => &[0x46, 0x55, 0x4A, 0x49, 0x46, 0x49, 0x4C, 0x4D], // FUJIFILM
            RawFormat::ORF => &[0x49, 0x49, 0x52, 0x4F], // IIRO (Olympus)
            RawFormat::PEF => &[0x49, 0x49, 0x2A, 0x00], // TIFF LE (Pentax)
            RawFormat::RW2 => &[0x49, 0x49, 0x55, 0x00], // IIU\0 (Panasonic)
            RawFormat::DNG => &[0x49, 0x49, 0x2A, 0x00], // TIFF LE (Adobe)
            // Standard formats
...
mod tests {
    ...
    fn test_raw_format_signature_matching() {
        let cr3_bytes = &[0x00, 0x00, 0x00, 0x18, 0x66, 0x74, 0x79, 0x70, 0x63, 0x72, 0x33, 0x20, 0x00];
        let raf_bytes = &[0x46, 0x55, 0x4A, 0x49, 0x46, 0x49, 0x4C, 0x4D, 0x00];
        let arw_bytes = &[0x49, 0x49, 0x2A, 0x00, 0x53];
