  ...
  return (
    <div className="flex items-center gap-1 px-1 py-0.5">
      <input ref={inputRef} type="text" value={value} onChange={(e) => setValue(e.target.value)} onKeyDown={handleKeyDown}
        ...
        placeholder="Nom de la collection..." maxLength={80} />
      <button onClick={() => { if (value.trim()) onConfirm(value.trim()); }} disabled={!value.trim()}
        className="text-emerald-400 hover:text-emerald-300 disabled:opacity-30 transition-colors" aria-label="Valider">
        <Check size={12} />
      </button>
      <button onClick={onCancel} className="text-zinc-500 hover:text-zinc-300 transition-colors" aria-label="Annuler">
        <X size={12} />
      </button>
    </div>
  );
...
function CollectionItem({ collection, isActive, onSelect, onDelete, onRename }: CollectionItemProps) {
  ...
  if (isEditing) {
    return (
      <div className="px-1 py-0.5">
        <input ref={inputRef} type="text" value={editValue} maxLength={80}
          onChange={(e) => setEditValue(e.target.value)}
          onKeyDown={(e) => { if (e.key === 'Enter') commitRename(); if (e.key === 'Escape') { setEditValue(collection.name); setIsEditing(false); } }}
          onBlur={commitRename}
  ...
  return (
    <div className={`flex items-center gap-1 text-[11px] rounded group transition-colors ${
      ...
      <button onClick={(e) => { e.stopPropagation(); setIsEditing(true); }}
        className="p-1 opacity-0 group-hover:opacity-50 hover:!opacity-100 text-zinc-500 hover:text-zinc-200 transition-all" aria-label={`Renommer ${collection.name}`}>
        <Pencil size={10} />
      </button>
      <button onClick={(e) => { e.stopPropagation(); onDelete(collection.id); }}
        className="p-1 opacity-0 group-hover:opacity-50 hover:!opacity-100 text-zinc-500 hover:text-red-400 transition-all" aria-label={`Supprimer ${collection.name}`}>
        <Trash2 size={10} />
      </button>
    </div>
  );
    ...
    <div className={`${sidebarOpen ? 'w-64' : 'w-0'} bg-zinc-900 border-r border-black flex flex-col shrink-0 transition-all duration-300 overflow-hidden`}>
      <div className="flex-1 overflow-y-auto p-3 custom-scrollbar">
        ...
        <section className="mb-8">
          <div className="text-[10px] font-black text-zinc-600 uppercase tracking-widest mb-4 px-2">Catalogue</div>
          <div className="space-y-0.5">
            <button onClick={handleClear} className={`w-full text-left p-2 rounded text-xs hover:bg-zinc-800 flex gap-2 transition-colors ${activeCollectionId === null ? 'text-zinc-200' : 'text-zinc-400'}`}>
            ...
            <button onClick={() => { handleClear(); onSetFilterText('star 5'); }} className="w-full text-left p-2 rounded text-xs hover:bg-zinc-800 flex gap-2 text-zinc-400 group">
              <Star size={14} className="text-amber-500 group-hover:scale-110 transition-transform" /> Meilleures Notes
            </button>
            <button onClick={() => { handleClear(); onSetFilterText('picked'); }} className="w-full text-left p-2 rounded text-xs hover:bg-zinc-800 flex gap-2 text-zinc-400">
        ...
        <section className="mb-8">
          <div className="flex justify-between items-center text-[10px] font-black text-zinc-600 uppercase tracking-widest mb-3 px-2">
            ...
            <button onClick={() => setShowNewInput(true)}
              ...
              aria-label="Créer une collection">+</button>
          </div>
          <div className="space-y-0.5">
            ...
            {showNewInput && (
              <NewCollectionInput onConfirm={(name) => void handleCreate(name)} onCancel={() => setShowNewInput(false)} />
            )}
            {staticCollections.length === 0 && !showNewInput && (
              <p className="text-[10px] text-zinc-700 italic px-2 py-1">Cliquez « + » pour créer une collection</p>
            )}
          </div>
        </section>
