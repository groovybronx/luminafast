...
impl Database {
    ...
    fn run_migration(&mut self, version: &str) -> DatabaseResult<()> {
        ...
        let migration_sql = match version {
            ...
            _ => return Err(DatabaseError::MigrationFailed(
                format!("Unknown migration version: {}", version)
            )),
        };
        
        log::info!("Migration SQL loaded, length: {} chars", migration_sql.len());
        ...
        for statement in migration_sql.split(';') {
            ...
            if !clean_statement.is_empty() {
                log::debug!("Executing: {}", &clean_statement[..100.min(clean_statement.len())]);
...
mod tests {
    ...
    fn test_migration_debug() -> DatabaseResult<()> {
        ...
        println!("First 200 chars: {}", &migration_sql[..200.min(migration_sql.len())]);
        ...
        for (i, statement) in statements.iter().enumerate() {
            ...
            if !statement.is_empty() && !statement.starts_with("--") {
                println!("Statement {}: {}", i, &statement[..100.min(statement.len())]);
                if i == 0 {
    ...
    fn test_manual_migration() -> DatabaseResult<()> {
        ...
        let table_exists: i64 = db.connection.query_row(
    ...
    fn test_migration_simple() -> DatabaseResult<()> {
        ...
        let tables_before: Vec<String> = db.connection()
        ...
        let tables_after: Vec<String> = db.connection()
            .prepare("SELECT name FROM sqlite_master WHERE type='table'")?
            .query_map([], |row| row.get(0))?
            .collect::<Result<_, _>>()?;
    ...
    fn test_migration_idempotency() -> DatabaseResult<()> {
        ...
        let migration_count: i64 = db.connection()
    ...
    fn test_insert_and_query_image() -> DatabaseResult<()> {
        ...
        // Query the image
        let image_count: i64 = db.connection()
