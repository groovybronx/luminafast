...
export const DevelopSliders = ({ activeImg, onDispatchEvent }: DevelopSlidersProps) => {
  // Track pending changes that haven't been committed to DB yet
  const pendingChangesRef = useRef<Partial<EditState>>({});

  const handleSliderChange = useCallback(
    (key: keyof EditState, value: number) => {
      // Update UI immediately (optimistic)
      onDispatchEvent('EDIT', { [key]: value });
      // Track this change as pending (not yet persisted to DB)
  ...
  const handleSliderCommit = useCallback(() => {
    // Only persist to DB if there are pending changes
    if (Object.keys(pendingChangesRef.current).length > 0) {
      onDispatchEvent('EDIT_COMMIT', pendingChangesRef.current);
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center text-[10px] font-black text-zinc-500 uppercase tracking-widest border-b border-zinc-800 pb-2">
        ...
        <RefreshCw
          size={12}
          className="text-zinc-700 hover:text-white cursor-pointer transition-colors"
        />
      </div>
        ...
        <div key={param.key} className="space-y-1 group">
          <div className="flex justify-between text-[10px] text-zinc-400 group-hover:text-zinc-200 transition-colors">
            <span className="flex items-center gap-2">
              <param.icon size={10} className="text-zinc-600" /> {param.label}
            </span>
            <span className="font-mono text-blue-500 font-bold">
              {activeImg.state.edits[param.key] > 0 ? '+' : ''}
              {activeImg.state.edits[param.key]}
            </span>
          </div>
          <input
            type="range"
