...
const Histogram = () => (
  <div className="h-24 bg-zinc-900 rounded border border-zinc-800 relative overflow-hidden flex items-end px-1 gap-[1px]">
    ...
    {Array.from({ length: 40 }).map((_, i) => {
      // Generate deterministic height based on index to avoid impure function calls
...
const LibraryGrid = ({ filteredImages, selection, toggleSelection, setActiveView, thumbnailSize }) => (
...
export default function LuminaApp() {
  ...
  const handleRating = useCallback((rating) => dispatchEvent('RATING_SET', { rating }), [dispatchEvent]);
  const handleFlag = useCallback((flag) => dispatchEvent('FLAG_SET', { flag }), [dispatchEvent]);
  ...
  // Performance logging moved outside useMemo to avoid impure function calls
  useLayoutEffect(() => {
    if (filterText.length > 2) {
      const start = performance.now();
      // Trigger the filtering by accessing filteredImages
      const result = filteredImages;
      const end = performance.now();
      // Use setTimeout to avoid cascading renders
      setTimeout(() => addLog(`DUCKDB: Scan in ${(end - start).toFixed(2)}ms. Results: ${result.length}`, 'duckdb'), 0);
    }
