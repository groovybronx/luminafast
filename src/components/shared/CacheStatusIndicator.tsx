...
export function CacheStatusIndicator() {
  ...
  // Color indicator based on hit rate
  let statusClass = 'cache-status--cold'; // < 50%
  if (parseFloat(l1HitRate) >= 80) {
    statusClass = 'cache-status--hot';
  } else if (parseFloat(l1HitRate) >= 50) {
  ...
  return (
    <div
      ...
      title={`L1: ${l1Util} (${l1HitRate}) | L2: ${l2Util}`}
    >
      📦 {l1Util} ({l1HitRate}){warmingIndicator}
    </div>
  );
}
