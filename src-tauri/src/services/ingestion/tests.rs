...
mod tests {
    use crate::models::discovery::{
        DiscoveredFile, IngestionResult, FileProcessingStatus,
        RawFormat, BatchIngestionRequest, BatchIngestionResult,
    };
    ...
    async fn test_ingest_single_file() {
        ...
        // Ingest the file
        let result: IngestionResult = service.ingest_file(&discovered_file).await.unwrap();
    ...
    async fn test_ingest_multiple_formats() {
        ...
        for file in &files {
            let result: IngestionResult = service.ingest_file(file).await.unwrap();
            results.push(result);
        }
        ...
        let formats: std::collections::HashSet<_> = results
            .iter()
            .map(|r| r.metadata.as_ref().unwrap().format_details.format)
            .collect();
    ...
    async fn test_duplicate_file_detection() {
        ...
        let result1: IngestionResult = service.ingest_file(&discovered_file).await.unwrap();
        assert!(result1.success);
        
        // Try to ingest the same file again
        let result2: IngestionResult = service.ingest_file(&discovered_file).await.unwrap();
        assert!(!result2.success);
        assert!(result2.error.is_some());
        assert!(result2.error.unwrap().contains("already exists"));
    }
    ...
    async fn test_batch_ingestion() {
        ...
        let result = service.batch_ingest(&request).await.unwrap();
    ...
    async fn test_basic_exif_extraction() {
        ...
        let exif = service.extract_basic_exif(&file_path).await.unwrap();
        ...
        assert_eq!(exif.make.unwrap(), "Unknown");
        assert_eq!(exif.model.unwrap(), "Unknown");
        assert_eq!(exif.iso.unwrap(), 100);
        assert_eq!(exif.aperture.unwrap(), 2.8);
        assert_eq!(exif.shutter_speed.unwrap(), "1/125");
        assert_eq!(exif.focal_length.unwrap(), 50.0);
        assert_eq!(exif.lens.unwrap(), "Unknown");
    }
    ...
    async fn test_file_hashing() {
        ...
        let exists1 = service.check_file_exists(&file1_path).await.unwrap();
        let exists2 = service.check_file_exists(&file2_path).await.unwrap();
        ...
        // Ingest first file
        let result1: IngestionResult = service.ingest_file(&discovered_file1).await.unwrap();
        assert!(result1.success);
    ...
    async fn test_database_record_creation() {
        ...
        let database_id = result.database_id.unwrap();
        
        // Verify image record was created (query only columns that exist in test schema)
        let (img_id, img_filename, img_extension, img_file_size): (i64, String, String, Option<i64>) = db
            .query_row(
    ...
    async fn test_update_discovered_file_status() {
        ...
        service.update_discovered_file_status(&mut discovered_file, &result).await.unwrap();
