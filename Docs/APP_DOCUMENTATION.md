...
- `wasmRenderingService.test.ts` : 35/35 ✅ (WASM wrapper + fallback + parite visuelle dataset + presets contrat M3.3)
...
**Contrat Parite Preview/Export (M3.3)** :

- Le backend compare maintenant sa sortie export (TIFF lossless) a un buffer preview de reference sur un jeu de presets communs (`low_light`, `highlights`, `high_contrast`, `skin_warm`, `mixed_interior_exterior`).
