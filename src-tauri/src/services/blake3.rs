...
impl Blake3Service {
    ...
    pub async fn hash_file(&self, file_path: &Path) -> HashResult<FileHash> {
        ...
        let meta = metadata(&file_path).await.map_err(|e| {
            match e.kind() {
    ...
    fn compute_hash_streaming(&self, file_path: &Path, file_size: u64) -> HashResult<FileHash> {
        ...
        let file = File::open(file_path).map_err(|e| {
            match e.kind() {
                ...
                std::io::ErrorKind::PermissionDenied => HashError::PermissionDenied(file_path.to_path_buf()),
        ...
        loop {
            let bytes_read = reader.read(&mut buffer).map_err(|e| {
                HashError::ReadError(file_path.to_path_buf(), e.to_string())
            })?;
    ...
    pub async fn verify_integrity(&self, file_path: &Path, expected_hash: &str) -> HashResult<bool> {
